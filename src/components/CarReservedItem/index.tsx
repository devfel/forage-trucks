// forage-trucks/src/components/CarReservedItem/index.tsx
...
const CarReservedItem: React.FunctionComponent<VehicleReservation> = ({ periodSelected, vehicle_id, avatar, bio, name, stringDate, nameSelected, reservationsList }) => {
  ...
  function convertBeginPeriodToMinutes(periodAM_PM: string): number {
    const periodMinutes =
      periodAM_PM.split(" to ")[0].split(" ")[1] === "PM"
  ...
  function convertEndPeriodToMinutes(periodAM_PM: string): number {
    const periodMinutes =
      periodAM_PM.split(" to ")[1].split(" ")[1] === "PM"
        ? //exeption for 12PM (noon)
  ...
  function checkAvailabilityPeriod(period1: string, period2: string): boolean {
    ...
    if ((period2.split(" to ")[0].split(" ")[0].split(":")[0] === "12" && period2.split(" to ")[0].split(" ")[1] === "AM") || (period2.split(" to ")[1].split(" ")[0].split(":")[0] === "12" && period2.split(" to ")[1].split(" ")[1] === "AM")) {
      return false;
    }
    ...
    <article className="car-item">
      ...
      <div className="text-info">
        ...
        <div className="reservations-list">
          <p>
            <b>Reservations List for {dReserved}</b>
          </p>
          {reservationsList
            .sort(function (a, b) {
              return convertBeginPeriodToMinutes(a.period) - convertBeginPeriodToMinutes(b.period);
            })
            .map((reservationItem: VehicleReservationsItem, index) => {
              return (
                <p key={reservationItem.reservation_id}>
                  <b>{index + 1}</b>: {reservationItem.staff_reserved} -{" "}
                  <i>
                    Period: <b>{reservationItem.period}</b>
                  </i>
                </p>
              );
            })}
